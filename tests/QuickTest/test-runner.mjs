...
check(
  "listFilterSum: filter (>2) [1..5] |> fold (+) 0",
  12,
  exp.testListFilterSum(),
);
check("listRevHead: rev [1;2;3] |> head", 3, exp.testListRevHead());
check(
  "listAppendLen: length (append [1;2] [3;4;5])",
  5,
  exp.testListAppendLen(),
);
...
// ─── List.tryFind ────────────────────────────────────────────────
check(
  "listTryFindSome: tryFind (>3) [1..5] -> Some 4 -> 4",
  4,
  exp.testListTryFindSome(),
);
check(
  "listTryFindNone: tryFind (>10) [1..5] -> None -> -1",
  -1,
  exp.testListTryFindNone(),
);
...
check(
  "refCellBool: ref 0, set 1 conditionally, .Value",
  1,
  exp.testRefCellBool(),
);
...
// ─── Recursive DU (Tree) ─────────────────────────────────────────
check(
  "treeSum: Node(Node(Leaf,Leaf,1),Node(Leaf,Leaf,2),3) = 6",
  6,
  exp.testTreeSum(),
);
...
check(
  "arrayZeroCreate: zeroCreate 4, arr.[2]<-7, arr.[2]: 7",
  7,
  exp.testArrayZeroCreate(),
);
...
check(
  "arrayFoldFloat: fold (+.) 0.0 [0.5,0.5,0.5] = 1.5",
  1.5,
  exp.testArrayFoldFloat(),
);
...
check(
  "arrayFilterAll: filter (>0) [1..5].Length = 5",
  5,
  exp.testArrayFilterAll(),
);
check(
  "arrayFilterNone: filter (>10) [1..5].Length = 0",
  0,
  exp.testArrayFilterNone(),
);
check("arrayExists: exists (>4) [1..5] = true → 1", 1, exp.testArrayExists());
check(
  "arrayExistsFalse: exists (>10) [1..5] = false → 0",
  0,
  exp.testArrayExistsFalse(),
);
check("arrayForAll: forall (>0) [1..5] = true → 1", 1, exp.testArrayForAll());
check(
  "arrayForAllFalse: forall (>3) [1..5] = false → 0",
  0,
  exp.testArrayForAllFalse(),
);
check("arrayInit: init 5 (i*i), [3] = 9", 9, exp.testArrayInit());
check(
  "arrayInitFold: init 5 id |> fold (+) 0 = 10",
  10,
  exp.testArrayInitFold(),
);
...
check(
  "arrayFindIndex: findIndex (>3) [1;5;2;4] = 1",
  1,
  exp.testArrayFindIndex(),
);
check("arrayContains: contains 3 [1..4] = 1", 1, exp.testArrayContains());
check(
  "arrayContainsFalse: contains 9 [1..4] = 0",
  0,
  exp.testArrayContainsFalse(),
);
...
check(
  "listCollectOrder: collect dup [1;2;3] sum = 18",
  18,
  exp.testListCollectOrder(),
);
check("listChoose: choose (>2) [1..4].Length = 2", 2, exp.testListChoose());
check(
  "listChooseSum: choose (*10 when >2) [1..4] sum = 70",
  70,
  exp.testListChooseSum(),
);

// ─── List.foldBack / sumBy / minBy / maxBy ────────────────────────────────────
check(
  "listFoldBack: foldBack (::) [1;2;3] [].Length = 3",
  3,
  exp.testListFoldBack(),
);
check(
  "listFoldBackOrder: foldBack (x-acc) [1;2;3] 0 = 2",
  2,
  exp.testListFoldBackOrder(),
);
...
check(
  "listContainsFalse: contains 9 [1..4] = 0",
  0,
  exp.testListContainsFalse(),
);
...
check(
  "arrayAppendLen: append [1;2;3] [4;5] length = 5",
  5,
  exp.testArrayAppendLen(),
);

// ─── String operations ────────────────────────────────────────────────────────
check(
  "stringIndexOf: 'hello world'.IndexOf('world') = 6",
  6,
  exp.testStringIndexOf(),
);
check(
  "stringIndexOfMiss: 'hello world'.IndexOf('xyz') = -1",
  -1,
  exp.testStringIndexOfMiss(),
);
check("stringStartsWith: starts 'hello' = 1", 1, exp.testStringStartsWith());
check(
  "stringStartsWithFalse: starts 'world' = 0",
  0,
  exp.testStringStartsWithFalse(),
);
check("stringEndsWith: ends 'world' = 1", 1, exp.testStringEndsWith());
check(
  "stringEndsWithFalse: ends 'hello' = 0",
  0,
  exp.testStringEndsWithFalse(),
);
check("stringSubstring: Substring(6).Length = 5", 5, exp.testStringSubstring());
check(
  "stringSubstringLen: Substring(6,3).Length = 3",
  3,
  exp.testStringSubstringLen(),
);

// ─── List.findIndex ───────────────────────────────────────────────────────────
check("listFindIndex: find (>3) [1;5;2;4] = 1", 1, exp.testListFindIndex());
check(
  "listTryFindIndex: tryFind (>10) [1;2;3] = -1",
  -1,
  exp.testListTryFindIndex(),
);

// ─── Option.Value ─────────────────────────────────────────────────────────────
check("optionValue: (Some 42).Value = 42", 42, exp.testOptionValue());
check(
  "optionMapValue: map (*2) (Some 42) .Value = 84",
  84,
  exp.testOptionMapValue(),
);
...
check(
  "resultBindError: bind on Error(-99) = -99",
  -99,
  exp.testResultBindError(),
);

// ─── String toLower / toUpper / trim / contains ───────────────────────────────
check(
  "toLowerLength: 'Hello'.ToLower().Length = 5",
  5,
  exp.testToLowerLength(),
);
check(
  "toUpperFirstChar: 'hello'.ToUpper()[0] = 72 ('H')",
  72,
  exp.testToUpperFirstChar(),
);
...
check(
  "toLowerMixed: 'HeLLo'.ToLower()[0] = 104 ('h')",
  104,
  exp.testToLowerMixed(),
);
...
check(
  "stringOfIntChar: string(123)[0] = 49 ('1')",
  49,
  exp.testStringOfIntChar(),
);
check(
  'stringInterpolation: $"x={42}".Length = 4',
  4,
  exp.testStringInterpolation(),
);
check(
  'stringInterpolationConcat: $"{3}+{7}".Length = 3',
  3,
  exp.testStringInterpolationConcat(),
);
...
check(
  "replace: 'hello world'.Replace('world','F#').Length = 8",
  8,
  exp.testReplace(),
);
check(
  "replaceRemove: 'aabbcc'.Replace('bb','').Length = 4",
  4,
  exp.testReplaceRemove(),
);
check(
  "replaceNoMatch: 'abc'.Replace('x','y').Length = 3",
  3,
  exp.testReplaceNoMatch(),
);
check(
  "printfnLiteral: printfn 'hello' returns 42",
  42,
  exp.testPrintfnLiteral(),
);

// ─── More List operations ──────────────────────────────────────────────────────
check("listItem: List.item 2 [10;20;30;40;50] = 30", 30, exp.testListItem());
check(
  "listTailHead: (List.tail [100;200;300]).Head = 200",
  200,
  exp.testListTailHead(),
);
...
check(
  "listReduceMax: reduce max [3;1;4;1;5;9;2;6] = 9",
  9,
  exp.testListReduceMax(),
);
check("listLast: List.last [1;2;3;4;5] = 5", 5, exp.testListLast());
check("listLastSingle: List.last [42] = 42", 42, exp.testListLastSingle());
check(
  "listSortDesc: (sortDescending [3;1;4;1;5;9]).Head = 9",
  9,
  exp.testListSortDesc(),
);
...
check(
  "isLetterOrDigit: IsLetterOrDigit('9') = 1",
  1,
  exp.testIsLetterOrDigit(),
);
...
check(
  "optionDefaultValue: defaultValue 99 (Some 42) = 42",
  42,
  exp.testOptionDefaultValue(),
);
check(
  "optionDefaultValueNone: defaultValue 99 None = 99",
  99,
  exp.testOptionDefaultValueNone(),
);
check(
  "optionDefaultWith: defaultWith (fun()->42) None = 42",
  42,
  exp.testOptionDefaultWith(),
);
check(
  "optionDefaultWithSome: defaultWith (fun()->99) (Some 5) = 5",
  5,
  exp.testOptionDefaultWithSome(),
);
check(
  "optionFilter: filter (>3) (Some 5) → value 5",
  5,
  exp.testOptionFilter(),
);
check(
  "optionFilterNone: filter (>10) (Some 5) → None (0)",
  0,
  exp.testOptionFilterNone(),
);
...
check(
  "duDataEqDiffCtor: Circle 3.0 = Square 3.0 → 0",
  0,
  exp.testDuDataEqDiffCtor(),
);
...
check(
  "identityFloat: identity<float> 2.5 > 2.0 → 1",
  1,
  exp.testIdentityFloat(),
);
check(
  "const2IntFloat: const2<int,float> 7 3.14 = 7",
  7,
  exp.testConst2IntFloat(),
);
check("const2IntInt: const2<int,int> 99 0 = 99", 99, exp.testConst2IntInt());
check(
  "const2FloatInt: const2<float,int> 2.5 42 > 2.0 → 1",
  1,
  exp.testConst2FloatInt(),
);

// ─── Sprint 9: fable-library-wasmgc/Map.fs — first F# library compiled by backend ──
check(
  "mapAddFind: MapModule.add 1 100 → tryFind 1 = 100",
  100,
  exp.testMapAddFind(),
);
check(
  "mapFindMissing: tryFind 99 on {1→100} = 0 (default)",
  0,
  exp.testMapFindMissing(),
);
check("mapCount: {1→100, 2→200} count = 2", 2, exp.testMapCount());
check(
  "mapContainsKey: containsKey 42 on {42→999} = 1",
  1,
  exp.testMapContainsKey(),
);
check(
  "mapContainsKeyMissing: containsKey 7 on {42→999} = 0",
  0,
  exp.testMapContainsKeyMissing(),
);
check(
  "mapAddReplace: add 1 555 over {1→100} → tryFind 1 = 555",
  555,
  exp.testMapAddReplace(),
);

// ─── Sprint 10b: fable-library-wasmgc/Option.fs — BCL migration (Phase A) ────
check(
  "optionModuleIsSome: OptionModule.isSome (Some 5) = 1",
  1,
  exp.testOptionModuleIsSome(),
);
check(
  "optionModuleIsSomeNone: OptionModule.isSome None = 0",
  0,
  exp.testOptionModuleIsSomeNone(),
);
check(
  "optionModuleIsNone: OptionModule.isNone None = 1",
  1,
  exp.testOptionModuleIsNone(),
);
check(
  "optionModuleIsNoneSome: OptionModule.isNone (Some 5) = 0",
  0,
  exp.testOptionModuleIsNoneSome(),
);
check(
  "optionModuleDefaultValueNone: defaultValue 42 None = 42",
  42,
  exp.testOptionModuleDefaultValueNone(),
);
check(
  "optionModuleDefaultValueSome: defaultValue 42 (Some 99) = 99",
  99,
  exp.testOptionModuleDefaultValueSome(),
);
check(
  "optionModuleCountNone: count None = 0",
  0,
  exp.testOptionModuleCountNone(),
);
check(
  "optionModuleCountSome: count (Some 5) = 1",
  1,
  exp.testOptionModuleCountSome(),
);

// ─── Sprint 10b: fable-library-wasmgc/Result.fs — BCL migration (Phase A) ────
check(
  "resultModuleIsOk: ResultModule.isOk (Ok 5) = 1",
  1,
  exp.testResultModuleIsOk(),
);
check(
  "resultModuleIsOkFalse: ResultModule.isOk (Error 0) = 0",
  0,
  exp.testResultModuleIsOkFalse(),
);
check(
  "resultModuleIsError: ResultModule.isError (Error 0) = 1",
  1,
  exp.testResultModuleIsError(),
);
check(
  "resultModuleIsErrorFalse: ResultModule.isError (Ok 5) = 0",
  0,
  exp.testResultModuleIsErrorFalse(),
);
check(
  "resultModuleDefaultValueError: defaultValue 42 (Error 0) = 42",
  42,
  exp.testResultModuleDefaultValueError(),
);
check(
  "resultModuleDefaultValueOk: defaultValue 42 (Ok 99) = 99",
  99,
  exp.testResultModuleDefaultValueOk(),
);
check(
  "resultModuleDefaultErrorOk: defaultError 42 (Ok 0) = 42",
  42,
  exp.testResultModuleDefaultErrorOk(),
);
check(
  "resultModuleDefaultErrorError: defaultError 42 (Error 7) = 7",
  7,
  exp.testResultModuleDefaultErrorError(),
);
...
check(
  "sprintfIntLiteral: sprintf 'x=%d' 100 length = 5",
  5,
  exp.testSprintfIntLiteral(),
);
check(
  "sprintfTwoInts: sprintf '%d %d' 3 7 length = 3",
  3,
  exp.testSprintfTwoInts(),
);
check(
  "sprintfTwoStrs: sprintf '%s and %s' 'foo' 'bar' length = 11",
  11,
  exp.testSprintfTwoStrs(),
);
check(
  "sprintfBoolTrue: sprintf '%b' true length = 4",
  4,
  exp.testSprintfBoolTrue(),
);
check(
  "sprintfBoolFalse: sprintf '%b' false length = 5",
  5,
  exp.testSprintfBoolFalse(),
);
check(
  "sprintfMixed: sprintf 'n=%d,s=%s' 7 'hi' length = 8",
  8,
  exp.testSprintfMixed(),
);
check(
  "sprintfZero: sprintf '%d' 0 first char = '0'=48",
  48,
  exp.testSprintfZero(),
);
check(
  "sprintfPrefix: sprintf 'Result: %d' 99 first char = 'R'=82",
  82,
  exp.testSprintfPrefix(),
);
check("sprintfFloat: sprintf '%f' 3.14 length >= 4", 4, exp.testSprintfFloat());
check(
  "sprintfFloatHalf: sprintf '%f' 0.5 length = 3",
  3,
  exp.testSprintfFloatHalf(),
);
check(
  "sprintfFloatWhole: sprintf '%f' 2.0 length = 3",
  3,
  exp.testSprintfFloatWhole(),
);
check(
  "sprintfFloatNeg: sprintf '%f' -1.5 length = 4",
  4,
  exp.testSprintfFloatNeg(),
);
check(
  "sprintfFloatInStr: sprintf 'pi=~%f' 3.14159 first char = 'p'=112",
  112,
  exp.testSprintfFloatInStr(),
);
check(
  "printfnInt: printfn 'count=%d' 42 doesn't crash",
  1,
  exp.testPrintfnInt(),
);
check(
  "printfnStr: printfn 'hello,%s!' 'world' doesn't crash",
  1,
  exp.testPrintfnStr(),
);
check(
  "printfnMulti: printfn '%d+%d=%d' 3 7 10 doesn't crash",
  1,
  exp.testPrintfnMulti(),
);
check("sprintfI: sprintf '%i' 99 length = 2", 2, exp.testSprintfI());
check(
  "sprintfPercent: sprintf '100%%' length = 4",
  4,
  exp.testSprintfPercent(),
);
check(
  "sprintfIntZeroChar: sprintf '%d' 0 char = 48",
  48,
  exp.testSprintfIntZeroChar(),
);
check(
  "sprintfEmptyStr: sprintf '%s' '' length = 0",
  0,
  exp.testSprintfEmptyStr(),
);
check(
  "sprintfIntFirstChar: sprintf '%d' 42 first char = '4'=52",
  52,
  exp.testSprintfIntFirstChar(),
);
check(
  "interpolationWithSprintf: $'val={99}' length = 6",
  6,
  exp.testInterpolationWithSprintf(),
);
